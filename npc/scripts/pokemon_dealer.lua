...
npcHandler:setCallback(CALLBACK_MESSAGE_DEFAULT, function(cid, type, msg)
    ...
    if (not isPokemonName(pokemonName, true) or table.find(FORBIDDEN_POKEMON, pokemonName)) then
        selfSay("Invalid Pokemon name.", cid)
        return true
    end

    local pokemonLevel = 5
    local pokemonExtraPoints = pokemonLevel + 5

    local ball = doCreatePokemonBall(cid, "ultra", pokemonName, pokemonLevel, nil, nil, pokemonExtraPoints, nil, nil, 99999999999, nil, getPokemonRandomSpecialAbility(pokemonName))
